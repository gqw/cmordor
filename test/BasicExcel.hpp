...
// Block handling functions
        bool Read(int index, char* block);
        bool Write(int index, const char* block);
        bool Swap(int index1, int index2);
        bool Move(int from, int to);
        bool Insert(int index, const char* block);
        bool Erase(int index);
        bool Erase(vector<int>& indices);
...
public:
        ...
        int FileSize(const wchar_t* path, int& size);
        int ReadFile(const wchar_t* path, char* data);
        int ReadFile(const wchar_t* path, vector<char>&data);
        int WriteFile(const wchar_t* path, const char* data, int size);
        int WriteFile(const wchar_t* path, const vector<char>&data, int size);
        ...
        int FileSize(const char* path, int& size);
        int ReadFile(const char* path, char* data);
        int ReadFile(const char* path, vector<char>& data);
        int WriteFile(const char* path, char* data, int size);
        int WriteFile(const char* path, vector<char>& data, int size);

// Protected functions and data members
protected:
        // General functions and data members
        void IncreaseLocationReferences(vector<int> indices);
        void DecreaseLocationReferences(vector<int> indices);
        ...
        void LoadBAT();
        void SaveBAT();
        int DataSize(int startIndex, bool isBig);
        int ReadData(int startIndex, char* data, bool isBig);
        int WriteData(const char* data, int size, int startIndex, bool isBig);
        void GetBlockIndices(int startIndex, vector<int>& indices, bool isBig);
        int GetFreeBlockIndex(bool isBig);
        void ExpandBATArray(bool isBig);
        void LinkBlocks(int from, int to, bool isBig);
        void FreeBlocks(vector<int>& indices, bool isBig);
        ...
        public:
                ...
                friend bool operator< (const CompoundFile::Property& lhs, const CompoundFile::Property& rhs)
                {
                        int maxLen1 = wcslen(lhs.name_);
                        int maxLen2 = wcslen(rhs.name_);
        ...
        PropertyTree* FindProperty(int index);
        PropertyTree* FindProperty(const wchar_t* path);
        PropertyTree* FindProperty(PropertyTree* parentTree, wchar_t* name);
        void InsertPropertyTree(PropertyTree* parentTree, Property* property, int index);
        void DeletePropertyTree(PropertyTree* tree);
        void UpdateChildrenIndices(PropertyTree* parentTree);
        void IncreasePropertyReferences(PropertyTree* parentTree, int index);
        void DecreasePropertyReferences(PropertyTree* parentTree, int index);
...
public:
        Record();
        virtual ~Record();
        virtual int Read(const char* data);
...
struct LargeString
{
        ...
        void Reset();
        int Read(const char* data);
        int ContinueRead(const char* data, int size);
        int Write(char* data);
        int DataSize();
        int RecordSize();
        int StringSize();
...
public:
        ...
        struct Index : public Record
        {
                ...
                virtual int RecordSize();
                int unused1_;
                int firstUsedRowIndex_;
                int firstUnusedRowIndex_;
                int unused2_;
                vector<int> DBCellPos_;
...
private: // Functions to read and write raw Excel format.
        int Read(const char* data, int dataSize);
        int Write(char* data);
...
public: // Cell functions
        int GetTotalRows();	///< Total number of rows in current Excel worksheet.
        int GetTotalCols();	///< Total number of columns in current Excel worksheet.
...
public:
        ...
        int GetStringLength() const;	///< Return length of ANSI or Unicode string (excluding null character).
