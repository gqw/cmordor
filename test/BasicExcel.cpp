...
// PROMISE: is present, 0 if otherwise.
{
        ...
        {
                int maxChildren = parentTree->children_.size();
                for (int i=0; i<maxChildren; ++i)
                {
...
// PROMISE: and next property will be readjusted to accomodate the deleted property.
{
        ...
        int maxChildren = tree->parent_->children_.size();
        int i;
        for (i=0; i<maxChildren; ++i)
        {
...
void CompoundFile::DecreasePropertyReferences(CompoundFile::PropertyTree* parentTree, int index)
...
Record::~Record() {};
int Record::Read(const char* data)
{
...
int Record::Write(char* data)
{
        ...
        {
                int maxContinue = continueIndices_.size();
                int size = continueIndices_[0];
                LittleEndian::Write(data, size, npos, 2); // Write size of record
                npos += 2;
...
int Record::DataSize() {return dataSize_;}
int Record::RecordSize() {return recordSize_;}

/************************************************************************************************************/

/************************************************************************************************************/
BOF::BOF() : Record() {code_ = CODE::BOF; dataSize_ = 16; recordSize_ = 20;}
int BOF::Read(const char* data)
{
        Record::Read(data);
...
int SmallString::Read(const char* data)
{
        Reset();
        char stringSize;
...
int SmallString::Write(char* data)
{
        int stringSize = 0;
        int bytesWrite = 0;
        if (unicode_ == 0)
...
int SmallString::DataSize() {return (unicode_ == 0) ? StringSize()+2 : StringSize()*2+2;}
int SmallString::RecordSize() {return DataSize();}
int SmallString::StringSize()
{
        if (unicode_ == 0)
        {
...
int LargeString::Read(const char* data)
{
        int stringSize;
...
int LargeString::ContinueRead(const char* data, int size)
{
        if (size == 0) return 0;
...
int LargeString::Write(char* data)
{
        int stringSize = 0;
        int bytesWrite = 0;
        if (unicode_ & 1)
...
int LargeString::DataSize()
{
        int dataSize = StringSize() + 3;
        if (richtext_) dataSize += 2 + 4*richtext_;
        if (phonetic_) dataSize += 4 + phonetic_;
        return dataSize;
}
int LargeString::RecordSize() {return DataSize();}
int LargeString::StringSize()
{
...
int Workbook::Read(const char* data)
{
        int bytesRead = 0;
        short code;
...
int Workbook::Write(char* data)
{
        ...
        int maxFonts = fonts_.size();
        {for (int i=0; i<maxFonts; ++i) {bytesWritten += fonts_[i].Write(data+bytesWritten);}}

        int maxXFs = XFs_.size();
        {for (int i=0; i<maxXFs; ++i) {bytesWritten += XFs_[i].Write(data+bytesWritten);}}

        int maxStyles = styles_.size();
        {for (int i=0; i<maxStyles; ++i) {bytesWritten += styles_[i].Write(data+bytesWritten);}}

        int maxBoundSheets = boundSheets_.size();
        {for (int i=0; i<maxBoundSheets; ++i) {bytesWritten += boundSheets_[i].Write(data+bytesWritten);}}
...
int Workbook::DataSize()
{
        ...
        int maxFonts = fonts_.size();
        {for (int i=0; i<maxFonts; ++i) {size += fonts_[i].RecordSize();}}

        int maxXFs = XFs_.size();
        {for (int i=0; i<maxXFs; ++i) {size += XFs_[i].RecordSize();}}

        int maxStyles = styles_.size();
        {for (int i=0; i<maxStyles; ++i) {size += styles_[i].RecordSize();}}

        int maxBoundSheets = boundSheets_.size();
        {for (int i=0; i<maxBoundSheets; ++i) {size += boundSheets_[i].RecordSize();}}
...
int Workbook::RecordSize() {return DataSize();}
/************************************************************************************************************/
...
int Workbook::Window1::Read(const char* data)
{
        Record::Read(data);
...
int Workbook::Window1::Write(char* data)
{
        data_.resize(dataSize_);
...
int Workbook::Font::Read(const char* data)
{
        Record::Read(data);
...
int Workbook::Font::Write(char* data)
{
...
int Workbook::Font::DataSize() {return (dataSize_ = 14 + name_.RecordSize());}
int Workbook::Font::RecordSize() {return (recordSize_ = DataSize()+4);}
/************************************************************************************************************/
...
int Workbook::XF::Read(const char* data)
{
        Record::Read(data);
...
int Workbook::XF::Write(char* data)
{
        data_.resize(dataSize_);
...
int Workbook::Style::Read(const char* data)
{
        Record::Read(data);
...
int Workbook::Style::Write(char* data)
{
        data_.resize(DataSize());
...
int Workbook::Style::DataSize() {return (dataSize_ = (XFRecordIndex_ & 0x8000) ? 4 : 2+name_.RecordSize());}
int Workbook::Style::RecordSize() {return (recordSize_ = DataSize()+4);}
/************************************************************************************************************/
...
int Workbook::BoundSheet::Read(const char* data)
{
        Record::Read(data);
...
int Workbook::BoundSheet::Write(char* data)
{
        data_.resize(DataSize());
...
int Workbook::BoundSheet::DataSize() {return (dataSize_ = 6+name_.RecordSize());}
int Workbook::BoundSheet::RecordSize() {return (recordSize_ = DataSize()+4);}
/************************************************************************************************************/
...
int Workbook::SharedStringTable::Read(const char* data)
{
        Record::Read(data);
        ...
        else
        {
                ...
                for (int i=0, c=0; i<uniqueStringsTotal_; ++i)
                {
                        ...
                        else
                        {
                                ...
                                {
                                        int dataSize = (continueIndices_[c] - continueIndices_[c-1] - 1) / multiplier;
...
int Workbook::SharedStringTable::Write(char* data)
{
        data_.resize(DataSize());
...
int Workbook::SharedStringTable::DataSize()
{
        dataSize_ = 8;
        continueIndices_.clear();
        int curMax = 8224;
        for (int i=0; i<uniqueStringsTotal_; ++i)
        {
                int stringSize = strings_[i].StringSize();
                ...
                else
                {
                        ...
                        if (curMax - dataSize_ >= 12)
                        {
                                ...
                                int additionalContinueRecords = unicode ? stringSize/8222 : stringSize/8223; // 8222 or 8223 because the first byte is for unicode identifier
                                for (int j=0; j<additionalContinueRecords; ++j)
                                {
                                        if (unicode)
                                        {
                                                --curMax;
...
int Workbook::SharedStringTable::RecordSize()
{
...
int Workbook::ExtSST::Read(const char* data)
{
        Record::Read(data);
        LittleEndian::Read(data_, stringsTotal_, 0, 2);

        int maxPortions = (dataSize_-2) / 8;
        streamPos_.clear();
        ...
        for (int i=0, npos=2; i<maxPortions; ++i)
        {
...
int Workbook::ExtSST::Write(char* data)
{
        data_.resize(DataSize());
        LittleEndian::Write(data_, stringsTotal_, 0, 2);

        int maxPortions = streamPos_.size();
        for (int i=0, npos=2; i<maxPortions; ++i)
        {
...
int Workbook::ExtSST::RecordSize() {return (recordSize_ = DataSize()+(int)((2+streamPos_.size()*8)/8224)*4)+4;}
/************************************************************************************************************/
...
int Worksheet::Read(const char* data)
{
        int bytesRead = 0;
        short code;
        LittleEndian::Read(data, code, 0, 2);
...
int Worksheet::RecordSize() {return DataSize();}
/************************************************************************************************************/
...
int Worksheet::Index::Read(const char* data)
{
        Record::Read(data);
        ...
        int nm = int(firstUnusedRowIndex_ - firstUsedRowIndex_ - 1) / 32 + 1;
        DBCellPos_.clear();
...
int Worksheet::Index::Write(char* data)
{
        data_.resize(DataSize());
...
int Worksheet::Index::DataSize() {return (dataSize_ = 16 + DBCellPos_.size()*4);}
int Worksheet::Index::RecordSize()
{
        int dataSize = DataSize();
        return (recordSize_ = dataSize + 4*(dataSize/8224 + 1));
}
...
int Worksheet::Dimensions::Read(const char* data)
{
        Record::Read(data);
...
int Worksheet::Dimensions::Write(char* data)
{
        data_.resize(DataSize());
...
int Worksheet::CellTable::RowBlock::CellBlock::Blank::Read(const char* data)
{
        Record::Read(data);
...
int Worksheet::CellTable::RowBlock::CellBlock::Blank::Write(char* data)
{
        data_.resize(DataSize());
...
int Worksheet::CellTable::RowBlock::CellBlock::BoolErr::Read(const char* data)
{
        Record::Read(data);
        LittleEndian::Read(data_, rowIndex_, 0, 2);
        LittleEndian::Read(data_, colIndex_, 2, 2);
...
int Worksheet::CellTable::RowBlock::CellBlock::BoolErr::Write(char* data)
{
        data_.resize(DataSize());
        LittleEndian::Write(data_, rowIndex_, 0, 2);
        LittleEndian::Write(data_, colIndex_, 2, 2);
...
int Worksheet::CellTable::RowBlock::CellBlock::LabelSST::Read(const char* data)
{
        Record::Read(data);
        LittleEndian::Read(data_, rowIndex_, 0, 2);
        LittleEndian::Read(data_, colIndex_, 2, 2);
...
int Worksheet::CellTable::RowBlock::CellBlock::LabelSST::Write(char* data)
{
        data_.resize(DataSize());
        LittleEndian::Write(data_, rowIndex_, 0, 2);
        LittleEndian::Write(data_, colIndex_, 2, 2);
...
int Worksheet::CellTable::RowBlock::CellBlock::MulBlank::Read(const char* data)
{
        Record::Read(data);
        LittleEndian::Read(data_, rowIndex_, 0, 2);
        LittleEndian::Read(data_, firstColIndex_, 2, 2);
        LittleEndian::Read(data_, lastColIndex_, dataSize_-2, 2);
        int nc = lastColIndex_ - firstColIndex_ + 1;
...
int Worksheet::CellTable::RowBlock::CellBlock::MulBlank::Write(char* data)
{
        data_.resize(DataSize());
        LittleEndian::Write(data_, rowIndex_, 0, 2);
        LittleEndian::Write(data_, firstColIndex_, 2, 2);
        LittleEndian::Write(data_, lastColIndex_, dataSize_-2, 2);
        int nc = XFRecordIndices_.size();
        for (int i=0; i<nc; ++i)
        {
...
int Worksheet::CellTable::RowBlock::CellBlock::MulBlank::DataSize() {return (dataSize_ = 6 + XFRecordIndices_.size()*2);}
int Worksheet::CellTable::RowBlock::CellBlock::MulBlank::RecordSize()
{
        int dataSize = DataSize();
        return (recordSize_ = dataSize + 4*(dataSize/8224 + 1));
}
...
int Worksheet::CellTable::RowBlock::CellBlock::MulRK::Read(const char* data)
{
        Record::Read(data);
        LittleEndian::Read(data_, rowIndex_, 0, 2);
        LittleEndian::Read(data_, firstColIndex_, 2, 2);
...
int Worksheet::CellTable::RowBlock::CellBlock::MulRK::Write(char* data)
{
        data_.resize(DataSize());
        LittleEndian::Write(data_, rowIndex_, 0, 2);
        LittleEndian::Write(data_, firstColIndex_, 2, 2);
...
int Worksheet::CellTable::RowBlock::CellBlock::MulRK::DataSize() {return (dataSize_ = 6 + XFRK_.size()*6);}
int Worksheet::CellTable::RowBlock::CellBlock::MulRK::RecordSize()
{
        int dataSize = DataSize();
        return (recordSize_ = dataSize + 4*(dataSize/8224 + 1));
}
...
int Worksheet::CellTable::RowBlock::CellBlock::Number::Read(const char* data)
{
        Record::Read(data);
        LittleEndian::Read(data_, rowIndex_, 0, 2);
        LittleEndian::Read(data_, colIndex_, 2, 2);
...
int Worksheet::CellTable::RowBlock::CellBlock::Number::Write(char* data)
{
        data_.resize(DataSize());
        LittleEndian::Write(data_, rowIndex_, 0, 2);
        LittleEndian::Write(data_, colIndex_, 2, 2);
...
int Worksheet::CellTable::RowBlock::CellBlock::RK::Read(const char* data)
{
        Record::Read(data);
        LittleEndian::Read(data_, rowIndex_, 0, 2);
        LittleEndian::Read(data_, colIndex_, 2, 2);
...
int Worksheet::CellTable::RowBlock::CellBlock::RK::Write(char* data)
{
        data_.resize(DataSize());
        LittleEndian::Write(data_, rowIndex_, 0, 2);
        LittleEndian::Write(data_, colIndex_, 2, 2);
...
int Worksheet::CellTable::RowBlock::CellBlock::Formula::Read(const char* data)
{
        Record::Read(data);
        LittleEndian::Read(data_, rowIndex_, 0, 2);
        LittleEndian::Read(data_, colIndex_, 2, 2);
...
int Worksheet::CellTable::RowBlock::CellBlock::Formula::Write(char* data)
{
        data_.resize(DataSize());
        LittleEndian::Write(data_, rowIndex_, 0, 2);
        LittleEndian::Write(data_, colIndex_, 2, 2);
...
int Worksheet::CellTable::RowBlock::CellBlock::Formula::DataSize() {return (dataSize_ = 18 + RPNtoken_.size());}
int Worksheet::CellTable::RowBlock::CellBlock::Formula::RecordSize()
{
        int dataSize = DataSize();
...
int Worksheet::CellTable::RowBlock::CellBlock::Formula::Array::Read(const char* data)
{
        Record::Read(data);
...
int Worksheet::CellTable::RowBlock::CellBlock::Formula::Array::Write(char* data)
{
        data_.resize(DataSize());
...
int Worksheet::CellTable::RowBlock::CellBlock::Formula::Array::DataSize() {return (dataSize_ = 12 + formula_.size());}
int Worksheet::CellTable::RowBlock::CellBlock::Formula::Array::RecordSize()
{
        int dataSize = DataSize();
        return (recordSize_ = dataSize + 4*(dataSize/8224 + 1));
}
...
int Worksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla::Read(const char* data)
{
        Record::Read(data);
        LittleEndian::Read(data_, firstRowIndex_, 0, 2);
        LittleEndian::Read(data_, lastRowIndex_, 2, 2);
...
int Worksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla::Write(char* data)
{
        data_.resize(DataSize());
        LittleEndian::Write(data_, firstRowIndex_, 0, 2);
        LittleEndian::Write(data_, lastRowIndex_, 2, 2);
...
int Worksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla::DataSize() {return (dataSize_ = 8 + formula_.size());}
int Worksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla::RecordSize()
{
        int dataSize = DataSize();
        return (recordSize_ = dataSize + 4*(dataSize/8224 + 1));
}
...
int Worksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla1::Read(const char* data)
{
        Record::Read(data);
        LittleEndian::Read(data_, firstRowIndex_, 0, 2);
        LittleEndian::Read(data_, lastRowIndex_, 2, 2);
...
int Worksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla1::Write(char* data)
{
        data_.resize(DataSize());
        LittleEndian::Write(data_, firstRowIndex_, 0, 2);
        LittleEndian::Write(data_, lastRowIndex_, 2, 2);
...
int Worksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla1::DataSize() {return (dataSize_ = 8 + formula_.size());}
int Worksheet::CellTable::RowBlock::CellBlock::Formula::ShrFmla1::RecordSize()
{
        int dataSize = DataSize();
        return (recordSize_ = dataSize + 4*(dataSize/8224 + 1));
}
...
int Worksheet::CellTable::RowBlock::CellBlock::Formula::Table::Read(const char* data)
{
        Record::Read(data);
        LittleEndian::Read(data_, firstRowIndex_, 0, 2);
        LittleEndian::Read(data_, lastRowIndex_, 2, 2);
...
int Worksheet::CellTable::RowBlock::CellBlock::Formula::Table::Write(char* data)
{
        data_.resize(DataSize());
        LittleEndian::Write(data_, firstRowIndex_, 0, 2);
        LittleEndian::Write(data_, lastRowIndex_, 2, 2);
...
int Worksheet::CellTable::RowBlock::CellBlock::Formula::String::Read(const char* data)
{
        Record::Read(data);
        string_.clear();
...
int Worksheet::CellTable::RowBlock::CellBlock::Formula::String::Write(char* data)
{
        data_.resize(DataSize());
        LittleEndian::WriteString(data_, &*(string_.begin()), 0, string_.size());
        return Record::Write(data);
}
int Worksheet::CellTable::RowBlock::CellBlock::Formula::String::DataSize() {return (dataSize_ = string_.size());}
int Worksheet::CellTable::RowBlock::CellBlock::Formula::String::RecordSize()
{
        int dataSize = DataSize();
        return (recordSize_ = dataSize + 4*(dataSize/8224 + 1));
}
...
int Worksheet::CellTable::RowBlock::Row::Read(const char* data)
{
        Record::Read(data);
        LittleEndian::Read(data_, rowIndex_, 0, 2);
...
int Worksheet::CellTable::RowBlock::Row::Write(char* data)
{
        data_.resize(DataSize());
        LittleEndian::Write(data_, rowIndex_, 0, 2);
...
Worksheet::CellTable::RowBlock::CellBlock::~CellBlock() {};
int Worksheet::CellTable::RowBlock::CellBlock::Read(const char* data)
{
        int bytesRead = 0;
...
int Worksheet::CellTable::RowBlock::CellBlock::Write(char* data)
{
        int bytesWritten = 0;
        switch (type_)
        {
...
int Worksheet::CellTable::RowBlock::CellBlock::DataSize()
{
        switch (type_)
        {
                case CODE::BLANK:
...
int Worksheet::CellTable::RowBlock::CellBlock::RecordSize()
{
        switch (type_)
        {
                case CODE::BLANK:
...
int Worksheet::CellTable::RowBlock::DBCell::Read(const char* data)
{
        Record::Read(data);
        LittleEndian::Read(data_, firstRowOffset_, 0, 4);
        int nm = (dataSize_-4) / 2;
        offsets_.clear();
        offsets_.resize(nm);
        for (int i=0; i<nm; ++i)
        {
...
int Worksheet::CellTable::RowBlock::DBCell::Write(char* data)
{
        data_.resize(DataSize());
        LittleEndian::Write(data_, firstRowOffset_, 0, 4);
        int nm = offsets_.size();
        for (int i=0; i<nm; ++i)
        {
...
int Worksheet::CellTable::RowBlock::DBCell::DataSize() {return (dataSize_ = 4+offsets_.size()*2);}
int Worksheet::CellTable::RowBlock::DBCell::RecordSize()
{
        int dataSize = DataSize();
        return (recordSize_ = dataSize + 4*(dataSize/8224 + 1));
}
...
/************************************************************************************************************/
int Worksheet::CellTable::RowBlock::Read(const char* data)
{
        int bytesRead = 0;
        short code;
        LittleEndian::Read(data, code, 0, 2);
...
int Worksheet::CellTable::RowBlock::Write(char* data)
{
        int bytesWritten = 0;
        int maxRows = rows_.size();
        {for (int i=0; i<maxRows; ++i)
        {
                bytesWritten += rows_[i].Write(data+bytesWritten);
        }}

        int maxCellBlocks = cellBlocks_.size();
        {for (int i=0; i<maxCellBlocks; ++i)
        {
...
int Worksheet::CellTable::RowBlock::DataSize()
{
        int dataSize = 0;
        int maxRows = rows_.size();
        {for (int i=0; i<maxRows; ++i) dataSize += rows_[i].RecordSize();}

        int maxCellBlocks = cellBlocks_.size();
        {for (int i=0; i<maxCellBlocks; ++i) dataSize += cellBlocks_[i].RecordSize();}

        dataSize += dbcell_.RecordSize();
        return dataSize;
}
int Worksheet::CellTable::RowBlock::RecordSize() {return DataSize();}
/************************************************************************************************************/

/************************************************************************************************************/
int Worksheet::CellTable::Read(const char* data)
{
        int bytesRead = 0;
...
int Worksheet::CellTable::Write(char* data)
{
        int bytesWritten = 0;
        int maxRowBlocks_ = rowBlocks_.size();
        for (int i=0; i<maxRowBlocks_; ++i)
        {
                bytesWritten += rowBlocks_[i].Write(data+bytesWritten);
        }
        return bytesWritten;
}
int Worksheet::CellTable::DataSize()
{
        int dataSize = 0;
        int maxRowBlocks_ = rowBlocks_.size();
        for (int i=0; i<maxRowBlocks_; ++i) dataSize += rowBlocks_[i].RecordSize();
        return dataSize;
}
int Worksheet::CellTable::RecordSize() {return DataSize();}
/************************************************************************************************************/
...
int Worksheet::Window2::Read(const char* data)
{
        Record::Read(data);
...
int Worksheet::Window2::Write(char* data)
{
        data_.resize(DataSize());
...
bool BasicExcel::Save()
{
        if (file_.IsOpen())
        {
                ...
                int minBytes = workbook_.RecordSize();
                int maxWorkSheets = worksheets_.size();
...
{
        int maxWorksheets = yesheets_.size();
        for (int i=0; i<maxWorksheets; ++i)
        {
...
BasicExcelWorksheet* BasicExcel::AddWorksheet(const char* name, int sheetIndex)
{
        ...
        else
        {
                ...
                int maxSheets = worksheets_.size();
...
int BasicExcel::Read(const char* data, int dataSize)
{
        int bytesRead = 0;
        BOF bof;
        short code;
...
int BasicExcel::Write(char* data)
{
        int bytesWritten = 0;
...
void BasicExcel::AdjustBoundSheetBOFPositions()
{
        ...
        int maxBoundSheets = workbook_.boundSheets_.size();
        for (int i=0; i<maxBoundSheets; ++i)
        {
...
void BasicExcel::AdjustDBCellPositions()
{
        ...
        for (int i=0; i<maxSheets; ++i)
        {
                ...
                int maxRowBlocks_ = worksheets_[i].cellTable_.rowBlocks_.size();
                for (int j=0; j<maxRowBlocks_; ++j)
                {
                        int firstRowOffset = 0;

                        int maxRows = worksheets_[i].cellTable_.rowBlocks_[j].rows_.size();
                        ...
                        int cellOffset = firstRowOffset - 20; // a ROW record is 20 bytes long

                        int maxCellBlocks = worksheets_[i].cellTable_.rowBlocks_[j].cellBlocks_.size();
                        {for (int k=0; k<maxCellBlocks; ++k)
                        {
...
{
        int offset = workbook_.bof_.RecordSize();
        offset += workbook_.bof_.RecordSize();
        offset += workbook_.window1_.RecordSize();

        int maxFonts = workbook_.fonts_.size();
        {for (int i=0; i<maxFonts; ++i) {offset += workbook_.fonts_[i].RecordSize();}}

        int maxXFs = workbook_.XFs_.size();
        {for (int i=0; i<maxXFs; ++i) {offset += workbook_.XFs_[i].RecordSize();}}

        int maxStyles = workbook_.styles_.size();
        {for (int i=0; i<maxStyles; ++i) {offset += workbook_.styles_[i].RecordSize();}}

        int maxBoundSheets = workbook_.boundSheets_.size();
        {for (int i=0; i<maxBoundSheets; ++i) {offset += workbook_.boundSheets_[i].RecordSize();}}

        workbook_.extSST_.stringsTotal_ = 10;
        int maxPortions = workbook_.sst_.uniqueStringsTotal_ / workbook_.extSST_.stringsTotal_ +
        ...
        for (int i=0; i<maxPortions; ++i)
        {
                ...
                for (int j=0; j<workbook_.extSST_.stringsTotal_; ++j)
                {
                        if (i*workbook_.extSST_.stringsTotal_+j >= workbook_.sst_.strings_.size()) break;
                        int stringSize = workbook_.sst_.strings_[i*workbook_.extSST_.stringsTotal_+j].StringSize();
                        ...
                        else
                        {
                                ...
                                if (8224 - relativeOffset >= 12)
                                {
                                        ...
                                        int additionalContinueRecords = stringSize / 8223; // 8223 because the first byte is for unicode
                                        for (int k=0; k<additionalContinueRecords; ++k)
                                        {
                                                stringSize -= 8223;
                                        }
...
{
        // Constants.
        const int maxWorksheets = yesheets_.size();
        ...
        map<vector<char>, int> stringMap;
        map<vector<char>, int>::iterator stringMapIt;
        map<vector<wchar_t>, int> wstringMap;
        map<vector<wchar_t>, int>::iterator wstringMapIt;
        ...
        {
                int maxRows = yesheets_[s].GetTotalRows();
                int maxCols = yesheets_[s].GetTotalCols();
                        ...
                        for (int c=0; c<maxCols; ++c)
                        {
                                ...
                                if (cellType != BasicExcelCell::UNDEFINED)	// Current cell contains some data
                                {
                                        if (worksheets_[s].index_.firstUsedRowIndex_ == 100000)
                                        {
                                                ...
                                                // Resize DBCellPos.
                                                int nm = int(worksheets_[s].index_.firstUnusedRowIndex_ - worksheets_[s].index_.firstUsedRowIndex_ - 1) / 32 + 1;
                                        ...
                                        switch(cellType)
                                        {
                                                ...
                                                case BasicExcelCell::STRING:
                                                {
                                                        ...
                                                        int maxUniqueStrings = workbook_.sst_.uniqueStringsTotal_;
                                                        int strIndex = 0;
                                                ...
                                                case BasicExcelCell::WSTRING:
                                                {
                                                        ...
                                                        // Check if string is present in Shared string table.
                                                        ++workbook_.sst_.stringsTotal_;
                                                        int maxUniqueStrings = workbook_.sst_.strings_.size();
                                                        int strIndex = 0;
...
{
        int maxWorksheets = excel_->workbook_.boundSheets_.size();
        for (int i=0; i<maxWorksheets; ++i)
        {
                        ...
                        switch (cell->Type())
                        {
                                ...
                                case BasicExcelCell::STRING:
                                {
                                        if (textQualifier != '\0')
                                                {
                                                // Get string.
                                                int maxLength = cell->GetStringLength();
...
int BasicExcelWorksheet::GetTotalRows()
{
        return maxRows_;
}

// Total number of columns in current Excel worksheet.
int BasicExcelWorksheet::GetTotalCols()
{
        return maxCols_;
}
...
BasicExcelCell* BasicExcelWorksheet::Cell(int row, int col)
{
        ...
        if (col>=maxCols_)
        {
                ...
                maxCols_ = col + 1;
                for (int i=0; i<maxRows_; ++i) cells_[i].resize(maxCols_);
        }
        if (row>=maxRows_)
        {
...
void BasicExcelWorksheet::UpdateCells()
{
        ...
        int maxRowBlocks = rRowBlocks.size();
        for (int i=0; i<maxRowBlocks; ++i)
        {
                vector<Worksheet::CellTable::RowBlock::CellBlock>& rCellBlocks = rRowBlocks[i].cellBlocks_;
                int maxCells = rCellBlocks.size();
                for (int j=0; j<maxCells; ++j)
                {
                        ...
                        switch (rCellBlocks[j].type_)
                        {
                                ...
                                case CODE::MULRK:
                                {
                                        int maxCols = rCellBlocks[j].mulrk_.lastColIndex_ - rCellBlocks[j].mulrk_.firstColIndex_ + 1;
                                        for (int k=0; k<maxCols; ++k, ++col)
                                        {
...
int BasicExcelCell::GetStringLength() const
{
